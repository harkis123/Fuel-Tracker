"""
Fuel Price Tracker v6.1 — PATCHED
Fix: Orlen LT sezoninė dyzelino klasė + teisingas stulpelio parinkimas

Pakeitimai nuo v6:
  1. parse_orlen_lt_pdf() — dinamiškai parenka dyzelino klasę pagal mėnesį
  2. Teisingas pardavimo kainos su PVM stulpelis (MAX reikšmė, ne paskutinis skaičius)
  3. Fallback: jei neranda konkrečios klasės, ieško bet kurio "Dyzelinas...RRME"
"""
...
#    🔧 PATCHED v6.1: sezoninė klasės detekcija
# ═══════════════════════════════════════

def get_diesel_search_terms(month=None):
    """
    Grąžina dyzelino klasių paiešką pagal mėnesį (EN 590 sezoninės klasės).
    
    Sezoninis grafikas Lietuvoje:
      Gegužė–Rugsėjis  → C klasė (vasarinis, CFPP -5°C)
      Balandis, Spalis  → E klasė (pereinamasis, CFPP -15°C)
      Lapkritis–Kovas   → 0/1/2 klasė (žieminis/arktinis, CFPP -20..-32°C)
    
    Grąžina sąrašą prioritetine tvarka — pirmas atitikimas naudojamas.
    """
    if month is None:
        month = TODAY.month
    ...
    if month in (5, 6, 7, 8, 9):
        # Vasarinis sezonas
        return [
            'Dyzelinas C kl',   # Pagrindinis — vasarinis
        ]
    elif month in (4, 10):
        # Pereinamasis sezonas
        return [
            'Dyzelinas E kl',   # Pagrindinis — pereinamasis
        ]
    else:
        # Žieminis sezonas (lapkritis–kovas)
        return [
            'Dyzelinas 2 kl',   # Arktinis (gruodis–vasaris)
            'Dyzelinas 1 kl',   # Žieminis
            'Dyzelinas 0 kl',   # Šalto klimato
            'Dyzelinas E kl',   # Fallback jei žieminis dar neprasidėjo
        ]
...
def parse_orlen_lt_pdf(pdf_bytes):
    ...
    Parso Orlen LT kainų protokolo PDF ir ištraukia pardavimo kainą su PVM.
    
    Pakeitimai:
    1. Dinamiškai parenka dyzelino klasę pagal sezoną
    2. Ima didžiausią skaičių eilutėje (= pardavimo su PVM), ne paskutinį
    3. Fallback per visas klases jei konkreti nerasta
    
    PDF eilutės struktūra (tipinė):
      Produktas | Bazinė kaina | Akcizas | Baz+akcizas | Pardavimo su PVM
      EUR/1000l    ~900-1200      ~504      ~1400-1700     ~1700-2100
    
    Pardavimo kaina su PVM = didžiausias skaičius eilutėje
    """
    try:
        import pdfplumber
    except ImportError:
    ...
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            # Nustatyti, kurios klasės ieškome
            search_terms = get_diesel_search_terms()
            log("Orlen LT", f"Searching for classes: {search_terms} (month={TODAY.month})")
            
            # 1. Pirma bandome pagal sezoninę klasę
            for search_term in search_terms:
                result = _find_diesel_price_in_text(text, search_term)
                if result:
                    result["diesel_class"] = search_term
                    return result
            
            # 2. Fallback: bet kuri "Dyzelinas...RRME" eilutė (ne arktinis, ne šildymui)
            log("Orlen LT", "Seasonal class not found, trying generic Dyzelinas+RRME fallback")
            result = _find_diesel_price_in_text(text, 'Dyzelinas', require_rrme=True)
            if result:
                result["diesel_class"] = "fallback"
                return result
            
            # 3. Paskutinis fallback: bet kuri "Dyzelinas" eilutė
            log("Orlen LT", "RRME fallback failed, trying any Dyzelinas")
            for line in text.split('\n'):
                if 'Dyzelinas' in line and 'šild' not in line.lower():
                    log("Orlen LT", f"Available line: {line[:120]}")
            
            log("Orlen LT", "No matching Dyzelinas line found in PDF", "WARN")
            return None
...
def _find_diesel_price_in_text(text, search_term, require_rrme=False):
    """
    Ieško konkrečios dyzelino eilutės PDF tekste ir ištraukia pardavimo kainą su PVM.
    
    Logika:
    - Randa eilutę su search_term (+ RRME jei reikia)
    - Ištraukia visus skaičius
    - Pardavimo kaina su PVM = DIDŽIAUSIAS skaičius (nes PVM prideda ~21%)
    - Validuoja kad kaina logiška (1000-2500 EUR/1000l)
    """
    for line in text.split('\n'):
        if search_term not in line:
            continue
        
        # Praleidžiame šildymo ir kitus nereikalingus produktus
        line_lower = line.lower()
        if 'šild' in line_lower or 'heat' in line_lower:
            continue
        
        # Jei reikia RRME
        if require_rrme and 'RRME' not in line and 'rrme' not in line_lower:
            continue
        
        log("Orlen LT", f"Found line: {line[:120]}")
        
        # Ištraukiame visus skaičius formatu "1 234.56" arba "1234.56"
        nums = re.findall(r'(\d[\d ]*\.\d{2,5})', line)
        cleaned = []
        for n in nums:
            val = float(n.replace(' ', ''))
            cleaned.append(val)
        
        log("Orlen LT", f"  Numbers found: {cleaned}")
        ...
        # ═══ PATAISYTA LOGIKA ═══
        # Pardavimo kaina su PVM = DIDŽIAUSIAS skaičius eilutėje
        # (nes: bazinė < bazinė+akcizas < pardavimo su PVM)
        
        # Filtruojame tik logiškas kainas (EUR/1000l diapazone)
        valid_prices = [v for v in cleaned if 1000 < v < 2500]
        
        if valid_prices:
            selling_price = max(valid_prices)  # 🔧 MAX vietoj [-1]
            eur_l = round(selling_price / 1000, 4)
            log("Orlen LT", f"  Pardavimo su PVM = {selling_price} EUR/1000l = {eur_l} EUR/l")
            
            # Papildoma info
            base_plus_excise = None
            if len(valid_prices) >= 2:
                sorted_prices = sorted(valid_prices)
                # Antras didžiausias = bazinė + akcizas (be PVM)
                base_plus_excise = sorted_prices[-2]
                log("Orlen LT", f"  Bazinė+akcizas = {base_plus_excise} EUR/1000l (be PVM)")
            
            return {
                "price_eur_l": eur_l,
                "price_eur_1000l_su_pvm": selling_price,
                "price_eur_1000l_be_pvm": base_plus_excise,
            }
        
        # Jei nėra validžių kainų 1000-2500 range, bandome mažesnius skaičius
        small_valid = [v for v in cleaned if 0.5 < v < 2.5]
        if small_valid:
            selling_price = max(small_valid)
            log("Orlen LT", f"  Pardavimo su PVM = {selling_price} EUR/l (jau EUR/l)")
            return {"price_eur_l": selling_price}
        
        log("Orlen LT", f"  No valid price found in numbers: {cleaned}", "WARN")
        break  # Found the line but couldn't parse
            ...
            for cname, cc in cc_names.items():
                if cname.lower() in cell0.lower():
                    ...
                    if val is not None:
                        try:
                            v = float(val)
                            if 0.5 < v < 3.5: countries[cc] = round(v, 4)
                            elif 500 < v < 3500: countries[cc] = round(v / 1000, 4)
...
def update_excel(fx=None, orlen_pl=None, orlen_lt=None, elvis_de=None, bsh_se=None, eu_bulletin=None, _failures=None):
    ...
    if "Daily Tracker" in wb.sheetnames:
        ...
        # 🔧 PATCHED: Show diesel class in notes
        diesel_class_info = ""
        if orlen_lt and orlen_lt.get("diesel_class"):
            diesel_class_info = f"|CLS:{orlen_lt['diesel_class']}"
        notes = f"Auto: {','.join(ok)}{diesel_class_info}"
        if _failures:
...
# ═══════════════════════════════════════
def main():
    print(f"\n{'='*60}\n FUEL PRICE TRACKER v6.1 — {TODAY_STR}\n{'='*60}\n")
    
    # 🔧 Show current diesel season
    search_terms = get_diesel_search_terms()
    print(f"  Diesel season: month={TODAY.month} → searching for {search_terms}")
    print()
